...
def _checkpoint_epoch(filename):
    parts = filename.split('.')
    if len(parts) >= 3 and parts[-1] == 'pth':
        try:
            return int(parts[-2])
        except ValueError:
            return None
...
def _prune_old_checkpoints(models_dir, task_prefix, keep_epoch):
    checkpoint_prefix = f"{task_prefix}_checkpoint."
    for filename in os.listdir(models_dir):
        if not filename.startswith(checkpoint_prefix) or not filename.endswith(".pth"):
            continue
        epoch = _checkpoint_epoch(filename)
        if epoch is None or epoch == keep_epoch:
            continue
...
def save_checkpoint(model, optimizer, scheduler, metrics_tracker, args, exp_dir,
    ...
    checkpoint_path = os.path.join(models_dir, f'{task_prefix}_checkpoint.{epoch}.pth')
    _atomic_torch_save(checkpoint, checkpoint_path)
    print(f"Saved {task_prefix} checkpoint for epoch {epoch} to {checkpoint_path}")

    # Save best checkpoint if needed
    if is_best:
        best_path = os.path.join(models_dir, f'{task_prefix}_best_checkpoint.pth')
        tmp_best_path = f"{best_path}.tmp.{os.getpid()}"
        try:
            os.link(checkpoint_path, tmp_best_path)
    ...
    _prune_old_checkpoints(models_dir, task_prefix, epoch)
